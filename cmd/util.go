...
func errReadProjNoPulumiYAML(projType projType, pwd string) error {
	...
	case pulumiPolicyProj:
		return fmt.Errorf("Could not find a Pulumi policy pack project (searching upwards for a "+
			"Pulumi.yaml file from directory %s). Try changing current directory to the root of "+
			"a policy pack program", pwd)

	default:
		return fmt.Errorf("Could not find a Pulumi app (searching upwards for a Pulumi.yaml "+
			"file form directory %s). If you have not created a project yet, use `pulumi new` to "+
			"do so", pwd)
	}
}
